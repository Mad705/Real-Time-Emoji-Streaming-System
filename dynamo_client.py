...
# Base server URL (change as needed)
...
# Global variables to hold server information and assigned cluster
...
def get_persistent_user_id():
    """
    Generates or retrieves a persistent user ID stored locally in a file.
    """
    if not os.path.exists(USER_ID_FILE):
        ...
        with open(USER_ID_FILE, 'r') as f:
            return json.load(f).get("user_id", random.randint(1, 1000))
...
def disconnect():
    ...
    Notifies the server and stops the emoji sending thread.
    """
    global send_data_flag
    print("Disconnected from server")
    # Notify the server via HTTP POST that this client has disconnected and needs to be deregistered
    if assigned_cluster:
        deregister_url = f"{BASE_SERVER_URL}/deregister"
        data = {'client_name': f"client_{user_id}"}
        try:
            response = requests.post(deregister_url, json=data)
            if response.status_code == 200:
                print(f"Client {user_id} successfully deregistered.")
            else:
                print(f"Error deregistering client: {response.status_code}")
        except Exception as e:
...
def register_client(client_name):
    ...
    Dynamically updates the assigned cluster.
    """
    global assigned_cluster
    try:
        response = requests.post(f"{BASE_SERVER_URL}/register", json={'client_name': client_name})
        if response.status_code == 200:
...
def main():
    ...
    # Step 2: Connect to the assigned cluster via Socket.IO
    try:
        sio.connect(BASE_SERVER_URL)
